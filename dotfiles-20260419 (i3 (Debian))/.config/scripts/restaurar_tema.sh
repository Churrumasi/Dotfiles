...
if [[ -f "$FONDO_VIDEO" ]]; then
    ...
    # Detectar resolución automáticamente
    RES=$(xrandr | grep '*' | awk '{print $1}')

    # Detectar entorno
    if [[ "$XDG_SESSION_TYPE" == "wayland" ]]; then
        # Wayland → usar mpvpaper si existe
        if command -v mpvpaper >/dev/null 2>&1; then
            pkill mpvpaper 2>/dev/null || true
            mpvpaper -o "no-audio loop" '*' "$FONDO_VIDEO" &>/dev/null &
        else
            echo "mpvpaper no instalado"
        fi
    else
        # X11 → usar xwinwrap (modo estable)
        xwinwrap -ov -g ${RES}+0+0 -- \
        mpv --no-audio --loop-file=inf --wid=%WID \
            --no-border --no-osc --no-osd-bar \
            --no-input-default-bindings --no-input-cursor \
            --keep-open=always --vo=x11 --hwdec=no \
