...
#############################################
# 2.5 Crop fullscreen 1600x1200 (Hyprland)
# Compatible con imágenes 4K en monitor 1080p
#############################################

if kdialog --yesno "¿Quieres crear un segundo fondo recortado (1600x1200)?" --title "Crop fullscreen"; then

    kdialog --msgbox "La imagen se mostrará fullscreen.\nSelecciona el área con el mouse."

    TMP_PREVIEW="/tmp/fondo_preview.png"

    #############################################
    # Obtener resolución del monitor activo
    #############################################

    read SCR_W SCR_H <<< $(hyprctl monitors -j | jq -r '.[] | select(.focused==true) | "\(.width) \(.height)"')

    #############################################
    # Crear preview ajustada a pantalla
    #############################################

    convert "$DEST_DIR/fondo.png" \
        -resize "${SCR_W}x${SCR_H}" \
        "$TMP_PREVIEW"

    #############################################
    # Mostrar imagen fullscreen
    #############################################
    ...
    if [[ -n "$GEOM" ]]; then
        ...
        if [[ "$W" -gt 50 && "$H" -gt 50 ]]; then

            #############################################
            # Resolución REAL de la imagen original
            #############################################

            read IMG_W IMG_H <<< $(identify -format "%w %h" "$DEST_DIR/fondo.png")

            #############################################
            # Calcular escala preview -> original
            #############################################

            SCALE_X=$(awk "BEGIN {print $IMG_W / $SCR_W}")
            SCALE_Y=$(awk "BEGIN {print $IMG_H / $SCR_H}")

            #############################################
            # Convertir coords pantalla -> coords reales
            #############################################

            REAL_X=$(awk "BEGIN {printf \"%d\", $X * $SCALE_X}")
            REAL_Y=$(awk "BEGIN {printf \"%d\", $Y * $SCALE_Y}")

            REAL_W=$(awk "BEGIN {printf \"%d\", $W * $SCALE_X}")
            REAL_H=$(awk "BEGIN {printf \"%d\", $H * $SCALE_Y}")
            ...
            kdialog --error "Selección inválida."
        fi

    else
        kdialog --error "No se realizó ninguna selección."
    fi
