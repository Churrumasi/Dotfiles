...
# Preguntar si desea ejecutar convertir.sh
zenity --question --text="¿Deseas ejecutar el script de conversión (convertir.sh) ahora?" --title="Ejecutar conversión"

if [[ $? -eq 0 ]]; then
    if [[ -x "$HOME/.config/scripts/convertir.sh" ]]; then
        "$HOME/.config/scripts/convertir.sh"
    else
        zenity --error --text="⚠️ El script convertir.sh no existe o no es ejecutable."
    fi
else
    zenity --info --text="⏭️ Conversión omitida."
fi
